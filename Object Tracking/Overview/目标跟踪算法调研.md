  ...
  * 现有的目标跟踪模型包括**生成模型**和**判别模型**
    ...
    * 最大后验概率 (Maximum A Posteriori, MAP) 估计
      ...
      * 公式
        ...
        * $$
          \mathop{argmax}\limits_{\theta}P\left(\theta|x_0,x_1,x_2,\dots,x_n\right)=\mathop{argmax}\limits_{\theta}P\left(x_0,x_1,x_2,\dots,x_n\right|\theta)\times P\left(\theta\right)
          $$
...
* 在实际跟踪过程中，需要考虑物体的外观等因素的影响, 同时以存在对象的 m 张图像作为参考可以显著提高滤波器模板的鲁棒性，故 **MOSSE 模型公式**如下
  $$
  min_{H^*}=\sum\limits_{i=1}^{m}\left|H^*F_i-G_i\right|^2
  $$
...
* CSK 线性分类器求解 CF 跟踪器表达式为
  ...
  * $$
    \mathop{min}\limits_{w}\sum\limits_{i}^{n}\left(f\left(x_i\right)-y_i\right)^2+\lambda||w||^2
    $$
...
* ==**循环矩阵 (Circulant Matrices)**==
  ...
  * 特性
    ...
    * $$
      X=F\mathop{diag}\left(\hat{x}\right)F^H=F\mathop{diag}\left(\hat{x}\right)F^{-T}\\
    ...
    * 如果**其他峰峰值与主峰峰值的比例大于阈值 APCE**，说明响应图是多峰模式，以这些多峰为中心重新检测，并取这些响应图的最大值作为最终目标位置
      ...
      * APCE
        ...
        * 公式
          ...
          * $$
            APCE=\frac{|F_{max}-F_{min}|^2}{\mathop{mean}\left(\sum_{w,h}\left(F_{w,h}-F_{min}\right)^2\right)}
            $$
...
* 类别
  ...
  * GNN (Graph Neural Networks):
    * 应用背景
      * CNN 和 RNN 被广泛用于处理欧几里德数据（例如 RGB 图像）或序列数据
      * 但其无法处理以**图形形式表示的数据**
    * 



正如我们之前讨论的，CNN 和 RNN 广泛用于处理欧几里德数据（例如 RGB 图像）或序列数据。 然而，这两个网络无法处理以图形形式表示的数据。 例如，在化学中，分子被表示为图，其生物活性需要被识别为在多个节点之间构建的边。 在引文网络中，文章通过引文相互链接，并且大多数文章可以分为不同的组。 因此，图数据的广泛应用场景给深度学习方法带来了巨大的挑战和机遇。

最近，人们提出了几种图神经网络（GNN）来解决上述挑战。 一般来说，图像可以被视为全连接图，其中所有像素都由相邻像素连接。 类似地，如图4所示，标准卷积层也可以看作是一种特殊的图卷积层，其中所有卷积核以无向方式连接[79]。 因此，可以发现，GNN 可以通过在传统 CNN 的内核中采用约束来实现。 戈里等人[80] 首先提出一种基于 GNN 的方法来处理具有不同特征的数据，例如有向图、无向图、标记图和循环图。 在这项开创性工作之后，Scarselli 等人[81, 82]发现，利用 GNN，可以通过迭代方式传播邻居信息直到达到稳定的固定点来获得单个节点的表示。 有了上述基础，图就被广泛嵌入到卷积层和神经网络中。 布鲁纳等人[83] 基于谱图理论开发图卷积。 在[84]中，提出了图内核来研究随机游走问题[85]，通过图内核可以使用映射函数将图或节点嵌入到特征空间中。

最近，研究人员发现欧几里得数据和序列数据也可以用特殊的图来表示。 因此，图神经网络也被用来处理这些数据[86−88]。 例如，为了捕获车道图的拓扑和远程依赖性，Liang 等人[86] 使用多个邻接矩阵和沿车道扩张扩展现有的图卷积。 通过将不同通道中的特征组织为节点，提出了代表性图层来动态采样最具代表性的特征[88]，从而减少计算消耗并获得更具代表性的特征。 由于不同数据的组成部分与其邻居稀疏或紧密相关，因此大多数数据可以以不同图的形式表示。 因此，未来可能的研究方向包括将非图数据转换为图，并使用 GNN 来处理这些转换后的图。
